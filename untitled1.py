...
CSV_FILE2 = '/Luana Mendes Alves - receitas_candidatos_2020_RJ.csv'
CSV_FILE3 = '/Luana Mendes Alves - despesas_contratadas_candidatos_2020_RJ.csv'
...
def convert_data(dataframe):
    ...
    for i in range(3, 9):
        dataframe.iloc[:, i] = label_encoders.fit_transform(dataframe.iloc[:, i])
...
def load_ready_ai_model(x, y):
    ...
    model = build_model(x_shape=x.shape, y_classes=y_classes, rnn_units=rnn_units)
...
def load_sidebar(data):
    ...
    estado_civil = np.unique(data['DS_GRAU_INSTRUCAO'])
    ...
    ocupacao = np.unique(data['DS_COR_RACA'])
...
def main():
    ...
    st.title('Luana - Previsão de Eleição')
    st.write('---')
    ...
    with open('/election_train_test_scaler.pkl', 'rb') as f:
